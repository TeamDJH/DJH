public class Clean {
	...
	public static void main(String[] args) {
		...
		// These are test string, one if the url is at the end of the string,
		// and other is url in the middle of string.
		...
		// fix this, won't work if the url your replacing is not the at the end
		// of the array
		// charArray[i] != 'e' &&
		while (charArray[i] != 'ê' && i != temp) {
			if (charArray[i] == 'h' && charArray[i + 1] == 't' && charArray[i + 2] == 't' && charArray[i + 3] == 'p') {
				// This assigns the location of the index to a temp variable
				// used in deleting urls.
				int j = i;
				markerStart = i;
				// This will erase contents of each element until it gets to a
				// white space, or end of the array.
				while (charArray[j] != ' ' && j != temp) {
					charArray[j] = 'ê';
					j++;
				}
				// Move back the original index so the original while loop sees
				// the first occurence of 'ê'
				i--;

				// If the url was not at the end of the string, we will try this
				// if statement
		...
		// If the url was in the middle then the markers will show when the url
		// starts and ends
		...
		if (markerEnd != temp) {
			...
			while (i != finalsize) {
				// ensures that the copy skips over the url and removes it for
				// the final copy.
				if (i == markerStart) {
					k = markerEnd + 1;
				}
		...
		// the url was at the end of the array, and therefore does not need the
		// markers
		else {
		...
		// This next part will convert the char array into a string then add
		// that string
